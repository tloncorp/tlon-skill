...
export function setCliCredentialOverrides(overrides: CliCredentialOverrides | null): void {
...
export function getCachedShips(): CachedAuth[] {
  return readCachedShipCandidates(getCacheDir(), nodeFs, { ignoreInvalid: true }).map(
    ({ cachePath: _cachePath, ...entry }) => entry
  );
}
...
export async function ensureClient(
  ...
  const deps: EnsureClientDeps = { ...defaultEnsureClientDeps, ...depsOverride };
  ...
  if (!initialized) {
    ...
    if (cfg.cookie) {
      await deps.configureClient({
        ...
        getCode: resolution.fallbackCode ? async () => resolution.fallbackCode as string : undefined,
      });
    ...
    if (didFreshAuth && resolution.mayWriteAuthCache) {
      ...
      if (freshCookie) {
        ...
        console.error(`Note: Credentials cached for ${preSig(cfg.ship)}. Next time run: tlon --ship ${preSig(cfg.ship)} <command>`);
      }
    }
