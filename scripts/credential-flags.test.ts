...
describe("credential flag parsing", () => {
  it("parses valid credential forms without mutating command args", () => {
    expect(parseGlobalCliOptions(["--config", "ship.json", "contacts", "self"])).toEqual({
      args: ["contacts", "self"],
      verbose: false,
      credentialOverrides: { kind: "config", configFile: "ship.json" },
    });

    expect(parseGlobalCliOptions(["--url", "https://zod", "--cookie", "urbauth-~zod=0v", "contacts"])).toEqual({
      args: ["contacts"],
      verbose: false,
      credentialOverrides: {
        kind: "cookie",
    ...
    expect(parseGlobalCliOptions(["--url", "https://zod", "--ship", "~zod", "--code", "code", "contacts"])).toEqual({
      args: ["contacts"],
      verbose: false,
      credentialOverrides: {
        kind: "code",
    ...
    expect(parseGlobalCliOptions(["--ship=~zod", "--verbose", "contacts"])).toEqual({
      args: ["contacts"],
      verbose: true,
  ...
  it("rejects duplicate, empty, and missing credential flag values", () => {
    expect(() => parseGlobalCliOptions(["--ship", "~zod", "--ship", "~bus", "contacts"])).toThrow(
      "Duplicate credential flag: --ship"
    );
    expect(() => parseGlobalCliOptions(["--cookie=", "contacts"])).toThrow("Missing value for --cookie");
    expect(() => parseGlobalCliOptions(["--url"])).toThrow("Missing value for --url");
    expect(() => parseGlobalCliOptions(["--url", "--cookie", "urbauth-~zod=0v", "contacts"])).toThrow(
      "Missing value for --url"
    );
    expect(() => parseGlobalCliOptions(["--url", "contacts", "self"])).toThrow("Missing value for --url");
  });
  ...
  it("leaves credential-like tokens after the command boundary untouched", () => {
    ...
    expect(parseGlobalCliOptions(["--verbose", "channels", "create", "~host/group", "--url"])).toEqual({
      args: ["channels", "create", "~host/group", "--url"],
      verbose: true,
      credentialOverrides: null,
    });

    expect(parseGlobalCliOptions(["contacts", "--config", "ship.json"])).toEqual({
