...
function memoryFs(files: Record<string, string>): ResolverFileSystem {
  return {
    exists(filePath) {
      ...
      return Object.keys(files).some((entry) => path.dirname(entry) === filePath);
    },
    readFile(filePath) {
...
function cacheFile(ship: string, url = `https://${ship}.tlon.network`, cookie = `urbauth-~${ship}=0v-cache`) {
...
describe("credential resolver", () => {
  it("lets CLI URL+cookie beat ambient TLON_CONFIG_FILE", () => {
    ...
    expect(result.config).toMatchObject({ url: "https://cli.tlon.network", ship: "zod" });
  ...
  it("lets CLI --ship plus TLON_SKILL_DIR beat ambient TLON_CONFIG_FILE and ship cache", () => {
    ...
    const result = resolveCredentials({
      ...baseInput(files, { TLON_CONFIG_FILE: "/tmp/env-config.json", TLON_SKILL_DIR: SKILL_DIR }),
      cli: { kind: "ship", ship: "~zod" },
    });
  ...
  it("fails partial ambient credential env instead of falling through", () => {
    expect(() => resolveCredentials(baseInput({}, { URBIT_URL: "https://zod.tlon.network" }))).toThrow(
      "Invalid ambient credentials"
    );
    expect(() => resolveCredentials(baseInput({}, { TLON_COOKIE: "urbauth-~zod=0v-cookie" }))).toThrow(
      "Invalid ambient credentials"
    );
    expect(() => resolveCredentials(baseInput({}, { TLON_SHIP: "~zod", TLON_CODE: "code" }))).toThrow(
      "Invalid ambient credentials"
    );
  });
  ...
  it("loads TLON_CONFIG_FILE with cookie credentials", () => {
    const files = {
      "/tmp/zod.json": json({ url: "https://zod.tlon.network", cookie: "urbauth-~zod=0v-cookie" }),
    };

    const result = resolveCredentials(baseInput(files, { TLON_CONFIG_FILE: "/tmp/zod.json" }));
  ...
  it("does not read cache when TLON_CONFIG_FILE provides code credentials", () => {
    const files = {
      "/tmp/zod.json": json({ url: "https://zod.tlon.network", ship: "~zod", code: "code" }),
  ...
  it("loads OpenClaw JSON fallback and preserves legacy JSON fallbacks", () => {
    ...
    const direct = resolveCredentials(
      baseInput({
        [openclawJson]: json({
          channels: { tlon: { url: "https://zod.tlon.network", cookie: "urbauth-~zod=0v-openclaw" } },
        }),
      })
    );
    const clawdbotLegacy = resolveCredentials(
      baseInput({
        [clawdbotLegacyJson]: json({
          channels: { tlon: { url: "https://bus.tlon.network", ship: "~bus", code: "legacy-code" } },
        }),
      })
    );
    const moltbotLegacy = resolveCredentials(
      baseInput({
        [moltbotLegacyJson]: json({
          channels: { tlon: { url: "https://nec.tlon.network", ship: "~nec", code: "moltbot-code" } },
        }),
      })
    );
  ...
  it("skips unusable default OpenClaw JSON and continues to later defaults", () => {
    ...
    expect(result.provenance.openclawPath).toBe(path.join(HOME, ".clawdbot", "moltbot.json"));
    expect(result.config.ship).toBe("bus");
  });

  it("treats explicit OPENCLAW_CONFIG parse errors and unusable JSON as local errors", () => {
    expect(() =>
      resolveCredentials(baseInput({ "/tmp/openclaw.txt": "not json" }, { OPENCLAW_CONFIG: "/tmp/openclaw.txt" }))
    ).toThrow("Failed to parse OpenClaw config");

    expect(() =>
      resolveCredentials(
        baseInput({ "/tmp/openclaw.json": json({ channels: {} }) }, { OPENCLAW_CONFIG: "/tmp/openclaw.json" })
      )
    ...
    expect(() =>
      resolveCredentials(
        baseInput(
          { "/tmp/openclaw.json": json({ channels: { tlon: { url: "${TLON_URL}", ship: "~zod", code: "code" } } }) },
    ...
    expect(() =>
      resolveCredentials(
        baseInput(
          { "/tmp/openclaw.json": json({ channels: { tlon: { url: "https://zod.tlon.network", code: "code" } } }) },
          { OPENCLAW_CONFIG: "/tmp/openclaw.json" }
        )
      )
    ...
    expect(() =>
      resolveCredentials(
        baseInput(
          { "/tmp/openclaw.json": json({ channels: { tlon: { url: "https://zod.tlon.network", ship: "~zod" } } }) },
          { OPENCLAW_CONFIG: "/tmp/openclaw.json" }
        )
      )
  ...
  it("does not let stale default YAML mask the real OpenClaw JSON path", () => {
    const files = {
      ...
      [path.join(HOME, ".openclaw", "openclaw.json")]: json({
        channels: { tlon: { url: "https://zod.tlon.network", ship: "~zod", code: "json-code" } },
      }),
    };
  ...
  it("fails targeted cache lookup when the cache file is invalid or mismatched", () => {
    expect(() => readCachedEntryForShip(CACHE_DIR, "~zod", memoryFs({ [getCachePath(CACHE_DIR, "zod")]: "not json" }))).toThrow(
      "Invalid cache entry"
    );
    expect(() =>
      readCachedEntryForShip(
        CACHE_DIR,
        "~zod",
        memoryFs({ [getCachePath(CACHE_DIR, "zod")]: json({ url: "https://zod.tlon.network", ship: "zod" }) })
      )
    ...
    expect(() =>
      readCachedEntryForShip(
        CACHE_DIR,
        "~zod",
        memoryFs({ [getCachePath(CACHE_DIR, "zod")]: json({ url: "https://zod.tlon.network", ship: "bus", cookie: "c" }) })
      )
    ...
    expect(() =>
      readCachedEntryForShip(
        CACHE_DIR,
        "~zod",
        memoryFs({ [getCachePath(CACHE_DIR, "zod")]: cacheFile("zod", "https://old.tlon.network") }),
  ...
  it("auto-selects exactly one cached ship and errors on multiple or duplicate cached ships", () => {
    const single = resolveCredentials(baseInput({ [getCachePath(CACHE_DIR, "zod")]: cacheFile("zod") }));
    expect(single.origin).toBe("single-cache");

    const singleWithInvalid = resolveCredentials(
      baseInput({
        [path.join(CACHE_DIR, "invalid.json")]: json({ url: "https://invalid.tlon.network", ship: "invalid" }),
        [getCachePath(CACHE_DIR, "zod")]: cacheFile("zod"),
      })
    );
  ...
  it("reports missing config when no source resolves", () => {
    expect(() => resolveCredentials(baseInput())).toThrow("Missing Urbit config");
  });
});
