...
function parseTargets(): Target[] {
  ...
  return targets.map((target) => {
    if (!isTarget(target)) {
      fail(`Unknown target ${target}. Supported targets: ${TARGETS.join(", ")}`);
    }
    return target;
  });
}
...
function copyRequired(source: string, destination: string, label: string): void {
...
function stageRootPackage(rootDir: string, stageDir: string): RootPackageJson {
  ...
  copyRequired(packageJsonPath, join(stageDir, "package.json"), "root package.json");
  copyRequired(join(rootDir, "bin", "tlon.js"), join(stageDir, "bin", "tlon.js"), "bin/tlon.js");
  copyRequired(
    join(rootDir, "scripts", "postinstall.js"),
    join(stageDir, "scripts", "postinstall.js"),
    "scripts/postinstall.js"
  );
  copyRequired(join(rootDir, "SKILL.md"), join(stageDir, "SKILL.md"), "SKILL.md");
  copyRequired(join(rootDir, "references"), join(stageDir, "references"), "references");
...
function stagePlatformPackage(
  ...
  copyRequired(sourcePackageJson, join(packageStageDir, "package.json"), `${target} package.json`);
...
const artifactsDir = resolve(rootDir, argValue("--artifacts-dir") ?? "artifacts");
...
try {
  ...
  const rootTarball = packDirectory(rootStageDir, outDir, cacheDir, userConfig, env);
  ...
  for (const target of targets) {
    const staged = stagePlatformPackage(rootDir, tempRoot, artifactsDir, target);
    const tarball = packDirectory(staged.packageStageDir, outDir, cacheDir, userConfig, env);
