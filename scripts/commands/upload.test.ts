...
function makeDeps(options: {
  ...
  const calls = {
    ...
    uploadFile: [] as Array<Parameters<UploadDeps["uploadApi"]["uploadFile"]>[0]>,
  };
...
describe("upload command run", () => {
  ...
  it("fails local usage errors before auth or IO", async () => {
    const cases = [
      ...
      { args: ["--definitely-not-an-option"], expected: "Unknown option: --definitely-not-an-option" },
      { args: ["-t"], expected: "-t requires a value" },
      { args: ["--stdin", "photo.jpg"], expected: "--stdin cannot be combined with a file or URL" },
    ];
  ...
  it("uploads stdin data with injected stdin and Blob construction", async () => {
    ...
    expect(context.calls.createBlob).toEqual([{ data: [4, 5, 6], contentType: "image/png" }]);
    ...
    expect((context.calls.uploadFile[0].blob as TestBlob).data).toEqual([4, 5, 6]);
  });
  ...
  it("uploads URL data with injected fetch", async () => {
    ...
    const exitCode = await run(["https://example.com/path/photo.jpg"], context.deps);
  ...
  it("uses MIME override for URL uploads", async () => {
    ...
    const exitCode = await run(["https://example.com/path/photo.bin", "--type", "image/webp"], context.deps);
  ...
  it("rejects blob read failures as expected command errors", async () => {
    ...
    expect(exitCode).toBe(1);
    expect(context.stdout()).toBe("");
    expect(context.stderr()).toBe("Error: Failed to read response body: body unavailable\n");
    expect(context.calls.authenticate).toBe(1);
    expect(context.calls.uploadFile).toEqual([]);
  });
  ...
  it("uploads local files with injected filesystem reads", async () => {
    ...
    expect(context.calls.createBlob).toEqual([{ data: [10, 11], contentType: "image/jpeg" }]);
    expect(context.calls.uploadFile[0]).toMatchObject({
      contentType: "image/jpeg",
      fileName: "photo.jpg",
    });
    expect((context.calls.uploadFile[0].blob as TestBlob).data).toEqual([10, 11]);
  });
  ...
  it("uses MIME override for local file uploads", async () => {
    ...
    const exitCode = await run(["/tmp/mystery", "-t", "image/webp"], context.deps);

    expect(exitCode).toBe(0);
    expect(context.calls.createBlob).toEqual([{ data: [9, 8, 7], contentType: "image/webp" }]);
    expect(context.calls.uploadFile[0]).toMatchObject({
      contentType: "image/webp",
      fileName: "mystery",
    });
  ...
  it("falls back to the default MIME type for unknown local extensions", async () => {
    ...
    expect(exitCode).toBe(0);
    expect(context.calls.createBlob).toEqual([{ data: [9, 8, 7], contentType: DEFAULT_CONTENT_TYPE }]);
