...
function parseArgs(args: string[]): ParsedUploadArgs {
  ...
  if (stdinMode) {
    if (positional.length > 0) {
      throw usageError("--stdin cannot be combined with a file or URL", UPLOAD_HELP);
    }
...
async function uploadFromUrl(
  ...
  if (!response.ok) {
    throw commandError(`Failed to fetch: ${response.status} ${response.statusText}`);
  }
...
async function uploadFromFile(
  ...
  const result = await deps.uploadApi.uploadFile({ blob, contentType: ct, fileName });
  return result.url;
}

async function uploadFromStdin(contentType: string, deps: UploadDeps): Promise<string> {
