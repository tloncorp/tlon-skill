...
function makeDeps(options: {
  ...
  const deps: ActivityDeps = {
    ...
    format: {
      ...
      groupUnread: (summary) => `GROUP:${summary.groupId}:${summary.count ?? 0}`,
      channelUnread: (summary) => `CHANNEL:${summary.channelId}:${summary.count ?? 0}`,
    },
  };
...
describe("activity command run", () => {
  ...
  it("fails local usage errors before auth or API work", async () => {
    const cases = [
      ...
      { args: ["mentions", "--limit", "abc"], expected: "--limit must be a positive integer" },
    ];
  ...
  it("leaves unexpected exceptions for the adapter formatter", async () => {
    ...
    await expect(run(["mentions"], context.deps)).rejects.toThrow("unexpected failure");
    expect(context.stderr()).toBe("");
  });
});
