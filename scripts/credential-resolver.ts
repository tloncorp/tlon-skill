...
export type ShipProvenance = "cli" | "env" | "cookie" | "config-file" | "skill-dir" | "openclaw" | "cache";
...
export type CliCredentialOverrides =
  ...
  | { kind: "cookie"; url: string; cookie: string; ship?: string; code?: string }
...
function resolutionFromTopLevelConfig(
  ...
  let ship = forcedShip ? normalizeShipName(forcedShip) : nonEmpty(data.ship) ? normalizeShipName(data.ship) : null;
  if (forcedShip && nonEmpty(data.ship) && normalizeShipName(data.ship) !== ship) {
    throw new Error(`Invalid config: ship ${withSig(data.ship)} does not match requested ship ${withSig(forcedShip)}`);
  }
  ...
  return makeResolution(
    ...
    {
      selectedBy,
      ship: forcedShip ? shipProvenance : derivedShip ? "cookie" : shipProvenance,
      configPath: filePath,
    },
...
function envAlias(
  ...
  return nonEmpty(env[urbitName]) ? env[urbitName] : nonEmpty(env[tlonName]) ? env[tlonName] : undefined;
}
...
function resolutionFromCookie(
  ...
  const derivedShip = ship ? normalizeShipName(ship) : parseShipFromCookie(cookie);
  if (!derivedShip) {
    throw new Error("Invalid config: must have ship (or cookie with ship in name)");
  }
...
function parseOpenClawConfig(
  ...
  try {
    ...
    throw new Error(`Failed to parse OpenClaw config ${filePath}: ${err.message}`);
  }
  ...
  if (!root.channels || !channels.tlon) {
    throw new Error(`OpenClaw config unusable: missing channels.tlon in ${filePath}`);
  }
  ...
  if (hasPlaceholder(url) || hasPlaceholder(code) || hasPlaceholder(cookie) || hasPlaceholder(rawShip)) {
    throw new Error(`OpenClaw config unusable: contains placeholder values in ${filePath}`);
  }
  if (!url) {
    throw new Error(`OpenClaw config unusable: missing tlon.url in ${filePath}`);
  }
  if (!code && !cookie) {
    throw new Error(`OpenClaw config unusable: must have tlon.code or tlon.cookie in ${filePath}`);
  }
  ...
  if (!ship) {
    throw new Error(`OpenClaw config unusable: must have tlon.ship or cookie ship in ${filePath}`);
  }
...
function resolveOpenClaw(input: CredentialResolverInput): CredentialResolution | null {
  ...
  const paths = input.openclawDefaultPaths ?? getDefaultOpenClawConfigPaths(input.homeDir);
...
function parseCacheFile(
  ...
  if (fileShip !== storedShip) {
    throw cacheError(`${filePath} filename ship ${withSig(fileShip)} does not match stored ship ${withSig(storedShip)}`);
  }

  if (expected?.ship && storedShip !== normalizeShipName(expected.ship)) {
    throw cacheError(`${filePath} stored ship ${withSig(storedShip)} does not match requested ship ${withSig(expected.ship)}`);
  }

  if (expected?.url && data.url !== expected.url) {
    throw cacheError(`${filePath} stored URL ${data.url} does not match requested URL ${expected.url}`);
  }
...
export function readCachedShipCandidates(
  ...
  let files: string[];
  try {
    files = fsDeps.readDir(cacheDir).filter((file) => file.endsWith(".json")).sort();
  } catch {
    return [];
  }
...
function resolutionFromCachedEntry(
  ...
  selectedBy: "cli" | "env" | "fallback" = origin === "ship-cache" ? "env" : "fallback"
): CredentialResolution {
  return makeResolution(
    {
      url: entry.url,
...
function resolveCli(input: CredentialResolverInput, cli: CliCredentialOverrides): CredentialResolution {
  switch (cli.kind) {
    case "config":
      return resolutionFromTopLevelConfig(cli.configFile, input.fs, "config-file", "cli", "config-file");
    case "cookie":
      return resolutionFromCookie("cli", cli.url, cli.cookie, cli.ship, cli.code, "cli", "cli");
    case "code":
      return resolutionFromCode("cli", cli.url, cli.ship, cli.code, "cli", "cli");
    case "ship":
      return resolveShipOnly(input, cli.ship, "cli", true) as CredentialResolution;
  }
}
...
export function resolveCredentials(input: CredentialResolverInput): CredentialResolution {
  if (input.cli) {
  ...
  if (nonEmpty(configFile)) {
    return resolutionFromTopLevelConfig(configFile, input.fs, "config-file", "env", "config-file");
  }
  ...
  if (url && cookie) {
    return resolutionFromCookie("env-cookie", url, cookie, ship, code, "env", ship ? "env" : "cookie");
  }
  ...
  const cachedShips = readCachedShipCandidates(input.cacheDir, input.fs, { ignoreInvalid: true });
  if (cachedShips.length === 1) {
    return resolutionFromCachedEntry(cachedShips[0], "single-cache");
  }
  if (cachedShips.length > 1) {
    const shipList = cachedShips.map((entry) => `  ${withSig(entry.ship)}`).join("\n");
