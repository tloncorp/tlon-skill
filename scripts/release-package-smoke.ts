...
function assertLockResolvedFromTarball(
  ...
  if (entry.version !== expectedVersion) {
    fail(`${key} version ${entry.version ?? "<missing>"} did not match ${expectedVersion}`);
  }
  if (!entry.resolved?.startsWith("file:")) {
    fail(`${key} did not resolve from a local file tarball: ${entry.resolved ?? "<missing>"}`);
  }
  if (!entry.resolved.includes(basename(tarballPath))) {
    fail(`${key} resolved to ${entry.resolved}, expected ${basename(tarballPath)}`);
  }
}
...
function assertNoRunnableNonNativePackages(projectDir: string, target: Target): void {
  for (const otherTarget of TARGETS) {
    if (otherTarget === target) {
      continue;
    }
    const packageDir = nodeModulesPackagePath(projectDir, PLATFORM_PACKAGES[otherTarget]);
    ...
    if (stat.isFile() && (stat.mode & 0o111) !== 0) {
      fail(`Non-native package ${PLATFORM_PACKAGES[otherTarget]} installed runnable binary ${binaryPath}`);
    }
  }
}
...
if (target !== runtimeTarget) {
  fail(`Package smoke target ${target} must match native runner ${runtimeTarget}`);
}

const tarballDir = resolve(rootDir, argValue("--tarball-dir") ?? "release-tarballs");
...
if (hashes[target]?.packageName !== nativePackageName) {
  fail(`binary-hashes.json package for ${target} did not match ${nativePackageName}`);
}
...
try {
  ...
  assertLockResolvedFromTarball(lock, packageJson.name, packageJson.version, rootTarball);
  assertLockResolvedFromTarball(lock, nativePackageName, packageJson.version, nativeTarball);
  ...
  const nativePackageDir = nodeModulesPackagePath(projectDir, nativePackageName);
